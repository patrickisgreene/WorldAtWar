use crate::{
    cli::EarthCli,
    core::{PreprocessError, PreprocessResult},
};
use gdal::{
...
impl PreprocessContext {
    pub fn from_cli(args: EarthCli) -> PreprocessResult<(Dataset, Self)> {
        let EarthCli {
            src_path,
            earth_path,
            temp_path,
    ...
    pub(crate) fn initialize(
        ...
        let tile_dir = earth_path.join(String::from(&attachment_label));
