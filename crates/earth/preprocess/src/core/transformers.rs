use crate::core::{
    GDALCustomTransformer, GDALTransformerInfo, PreprocessError, PreprocessResult, Transformer,
};
...
use std::ffi::c_void;
use std::ptr;
use waw_earth_render::math::Coordinate;
...
impl ReprojectionTransformer {
    pub fn from_ref(src_spatial_ref: &SpatialRef, dst_spatial_ref: &SpatialRef) -> PreprocessResult<Self> {
        let ptr = unsafe {
    ...
    pub fn from_dataset(
        ...
        Ok(GDALCustomTransformer {
            ...
            inner: Box::new(Self {
                ...
                dst_geo_transform,
                lon_lat_transformer: ReprojectionTransformer::from_ref(
                    &src.spatial_ref()?,
