...
use bevy::{
    ...
    input::{ButtonInput, mouse::{AccumulatedMouseMotion, MouseWheel}},
...
pub fn orbital_camera_controller(
    ...
    if mouse_buttons.pressed(MouseButton::Left) {
        if let Some(cursor_position) = cursor_position && controller.pan_data.is_none() {
        ...
        if let Some(cursor_position) = cursor_position {
            if controller.rotation_data.is_none() {
                ...
                controller.rotation_data = Some(RotationData {
                    ...
                    rotation: DVec2::ZERO,
                    initial_tilt: (controller.anchor_position - earth_origin)
    ...
    // Scroll wheel zoom
    let scroll_delta: f32 = mouse_wheel.read().map(|ev| ev.y).sum();
    ...
    if scroll_delta != 0.0 {
        if let Some(cursor_position) = cursor_position {
            ...
            if let Some(data) = controller.zoom_data.as_mut() {
                data.target_zoom -= scroll_delta as f64 * zoom_speed;
                data.zoom = data.zoom.lerp(data.target_zoom, smoothing);
            }
        }
    } else if controller.zoom_data.is_some() {
        // Continue smoothing to target
        if let Some(data) = controller.zoom_data.as_mut() {
            data.zoom = data.zoom.lerp(data.target_zoom, smoothing);

            // Clear zoom data when we've reached the target
            if (data.zoom - data.target_zoom).abs() < 0.001 {
    ...
    if let Some(pan_data) = controller.pan_data {
        ...
        // The earth is just rotated, but not translated relative to the camera.
        ...
        let radius = (controller.anchor_position - earth_origin).length();
        ...
        let initial_direction = (controller.anchor_position - earth_origin).normalize();
        let new_direction = (new_cursor_position - earth_origin).normalize();
        ...
        new_camera_position = earth_origin + rotation * (controller.camera_position - earth_origin);
    ...
    if let Some(rotation_data) = controller.rotation_data {
        ...
        let heading_axis = (controller.anchor_position - earth_origin).normalize(); // earth normal
    ...
    if let Some(zoom_data) = controller.zoom_data {
        // Invariants:
        // The earth origin stays at the screen center.
        // The cursor world position stays at the same screen-space location.

        let anchor_earth = controller.anchor_position - earth_origin;
        let camera_earth = earth_origin - controller.camera_position;
        let camera_anchor = controller.anchor_position - controller.camera_position;

        // compute the side lengths and the angles of the triangle anchor - earth origin - new camera
        let a = anchor_earth.length();
        let b = 2.0_f64.powf(zoom_data.zoom);

        let alpha = camera_earth.angle_between(camera_anchor);
        ...
        let rotation = DQuat::from_axis_angle(camera_earth.cross(camera_anchor).normalize(), beta);

        let camera_position = earth_origin + rotation * (c * anchor_earth.normalize());

        let initial_direction = camera_earth.normalize();
        let new_direction = (earth_origin - camera_position).normalize();
